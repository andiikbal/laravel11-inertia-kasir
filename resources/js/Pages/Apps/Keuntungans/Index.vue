            ...
            <div class="card border-0 rounded-3 shadow border-top-purple">
              <div class="card-header">
                <span class="font-weight-bold">
                  <i class="fa fa-chart-line"></i> LAPORAN KEUNTUNGAN
                </span>
              </div>
              <div class="card-body">
                ...
                <div v-if="keuntungans.length > 0">
                  <hr />
                  <div class="export text-end mb-3">
                    <a
                      :href="`/apps/keuntungans/export?tanggal_awal=${form.tanggal_awal}&tanggal_akhir=${form.tanggal_akhir}`"
                      target="_blank"
                    ...
                    <a
                      :href="`/apps/keuntungans/pdf?tanggal_awal=${form.tanggal_awal}&tanggal_akhir=${form.tanggal_akhir}`"
                      target="_blank"
                      class="btn btn-secondary btn-md border-0 shadow shadow me-3"
                    >
                      <i class="fa fa-file-pdf"></i> PDF
                    </a>
                    <a
                      :href="`/apps/keuntungans`"
                      class="btn btn-warning btn-md border-0 shadow shadow"
                    >
                      RESET
                    </a>
                  </div>
                  <table class="table table-bordered">
                    ...
                    <tbody>
                      <tr
                        v-for="keuntungan in keuntungans"
                        :key="keuntungan.id"
                      >
                        <td>{{ keuntungan.created_at }}</td>
                        <td class="text-center">
                          {{ keuntungan.transaksi.faktur }}
                        </td>
                        <td class="text-end">
                          Rp. {{ formatPrice(keuntungan.total) }}
                        </td>
                      </tr>
                      <tr>
                        <td
  ...
  setup(props) {
    ...
    const filter = () => {
      ...
      if (form.tanggal_awal == "" && form.tanggal_akhir == "") {
        ...
        axios
          .post("/apps/keuntungans", {
            tanggal_awal: form.tanggal_awal,
            tanggal_akhir: form.tanggal_akhir,
          })
          .then((response) => {
            keuntungans.value = response.data.data.keuntungans;
